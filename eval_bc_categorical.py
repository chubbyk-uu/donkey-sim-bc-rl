...
from train_bc_categorical import CategoricalSteeringModel
...
def run_episode(env, model, device, bin_centers, args, episode_index: int, global_step_start: int):
    obs, info = env.reset()
...
def main() -> None:
    ...
    if checkpoint.get("head_type") != "categorical_steer_v1":
        raise ValueError(f"unsupported checkpoint head_type: {checkpoint.get('head_type')}")

    bin_centers_np = np.asarray(checkpoint["bin_centers"], dtype=np.float32)
    bin_centers = torch.tensor(bin_centers_np, dtype=torch.float32, device=device)
    model = CategoricalSteeringModel(num_bins=len(bin_centers_np)).to(device)
    ...
    for episode_index in range(args.episodes):
        ...
        try:
            summary = run_episode(env, model, device, bin_centers, args, episode_index, global_step)
